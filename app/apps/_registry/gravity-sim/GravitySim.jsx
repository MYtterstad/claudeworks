...
function generateSystem(
  ...
  // Minimum distance of 120 from center to give planets room to orbit before any merge
  for (let i = 0; i < numBodies; i++) {
    // Distribute radii more evenly: inner edge at 120, spread planets across 120-400
    const r = randomRange(120, 400)
  ...
  // Assign velocities
  if (orbitalMode) {
    // Orbital: compute proper tangential velocity via cross product with a
    // random reference vector. This gives a clean perpendicular direction
    // regardless of the planet's position, avoiding degenerate axis selection.
    const sun = bodies[0]
    for (let i = 1; i < bodies.length; i++) {
      ...
      if (dist > 0.1) {
        // Circular orbital speed with slight perturbation (0.9-1.1x)
        const orbitalSpeed = Math.sqrt((G * sun.mass) / dist)
        const perturbation = randomRange(0.9, 1.1)
        const speed = orbitalSpeed * perturbation

        // Pick a random reference vector not parallel to radius
        let rx = Math.random() - 0.5, ry = Math.random() - 0.5, rz = Math.random() - 0.5
        // Cross product: radius × random → tangent
        let tx = dy * rz - dz * ry
        let ty = dz * rx - dx * rz
        let tz = dx * ry - dy * rx
        let tlen = Math.sqrt(tx * tx + ty * ty + tz * tz)
        // If degenerate (parallel vectors), pick a different reference
        if (tlen < 0.001) {
          rx = 1; ry = 0; rz = 0
          tx = dy * rz - dz * ry
          ty = dz * rx - dx * rz
