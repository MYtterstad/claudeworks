// App registry — metadata and documentation for all apps.
// Component loading is handled separately via next/dynamic.
...
The app uses real-time map visualization with Leaflet and OpenStreetMap, distance-optimized route generation through street networks, browser geolocation for automatic positioning, and live POI discovery from geographic databases.`,
    path: '/apps/runner',
    preview: '/previews/runner.svg',
    version: '0.2',
    language: 'jsx',
    tags: ['React', 'Maps', 'Navigation', 'Leaflet', 'OSM'],
    glowColor: 'var(--glow-green)',
...
Features include portfolio overview with project status tracking, project detail view with phase timeline editing, inline field editing with audit logging, and automatic Captario JSON export for financial modeling. Fully integrated with development phase data including probability of success per phase and multi-regional commercial assumptions.`,
    path: '/apps/ppm',
    preview: '/previews/ppm.svg',
    version: '1.0',
    language: 'jsx',
    tags: ['React', 'Pharma', 'Portfolio Management', 'Database'],
    glowColor: 'var(--glow-blue)',
  ...
  },
  {
    ...
    categoryType: 'commercial',
    listed: false,
    summary: 'Interactive Titan Portfolio Monte Carlo Report',
    description: `A comprehensive portfolio analysis report for the Titan Portfolio, featuring Monte Carlo simulation results across 10,000 iterations. Includes NPV distribution, launch probability analysis, decision trees, revenue projections, and P&L statements.

Tabs include Dashboard (KPIs, distributions), Projects (sortable table with eNPV, P(Launch), ROI), Project Detail (commercial model, cash flow, decision tree), Cash Flow (waterfall), Revenue Analysis (fan chart with uncertainty bands), P&L Statement, Gantt (timeline with milestones), Pipeline (phase columns with color-by), Decision Trees (per-project and portfolio-level), and Portfolio Timeline.

Built with React and Recharts. All data is embedded — this is a self-contained analytical report generated from Captario simulation output.`,
    path: '/apps/ppm-viewer',
    preview: '/previews/ppm.svg',
    version: '1.0',
    language: 'jsx',
    tags: ['React', 'Recharts', 'Pharma', 'Monte Carlo', 'Portfolio Analysis'],
