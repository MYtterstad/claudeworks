...
function createAnt(nestX, nestY, rng) {
  // Give each ant a random initial direction so they fan out from the nest
  const initDir = DIR8[Math.floor(rng() * DIR8.length)]
  return {
    x: nestX, y: nestY, state: 'foraging', prevDx: initDir[0], prevDy: initDir[1],
...
function computeAntScores(ant, grid, foods, nestX, nestY, params) {
  ...
  // Check if there's any meaningful pheromone nearby (foraging ants only)
  let hasPheromoneTrail = false
  if (ant.state === 'foraging') {
    for (let di = 0; di < DIR8.length; di++) {
      const nx = ant.x + DIR8[di][0], ny = ant.y + DIR8[di][1]
      if (isWalkable(grid, nx, ny) && pherLayer[gIdx(grid, nx, ny)] > 1.0) { hasPheromoneTrail = true; break }
    }
  }
  ...
  for (let di = 0; di < DIR8.length; di++) {
    ...
    // Exploration drive: when foraging with no pheromone trail and no food detected,
    // ants should radiate AWAY from the nest — like real ants fanning out to scout.
    // This is the key to reaching distant food sources.
    let exploration = 1
    if (ant.state === 'foraging' && !hasTarget && !hasPheromoneTrail) {
      const ndx = ant.x - nestX, ndy = ant.y - nestY
      const nDist = Math.sqrt(ndx*ndx + ndy*ndy) || 1
      // Dot product of move direction with away-from-nest direction
      const awayDot = (dx * ndx + dy * ndy) / (Math.SQRT2 * nDist)
      // Strong outward bias: 3x for moving away, 0.2x for moving toward nest
      exploration = Math.pow(clamp((awayDot + 1) / 2, 0.05, 1), 1.5) * 3
    }

    let momentum = 1
    if (ant.prevDx !== 0 || ant.prevDy !== 0) {
      const mdot = dx*ant.prevDx + dy*ant.prevDy
      // Strong momentum: reward continuing forward, strongly penalize reversals
      momentum = mdot > 0 ? 2.5 : mdot === 0 ? 0.5 : 0.1
    }
    const score = tauPow * etaPow * momentum * exploration
    scores.push({ dx, dy, label: DIR_LABELS[di], walkable: true, tau: tauPow, eta: etaPow, mom: momentum, score })
  }
  // Minimal exploration floor — just prevents zero-probability cells
  const maxScore = scores.reduce((m, s) => s.walkable && s.score > m ? s.score : m, 0)
  const floor = Math.max(maxScore * 0.005, 0.0001)
...
function stepAnt(ant, grid, foods, nestX, nestY, params, rng) {
  ...
  if (isStagnant && rng() < 0.3) {
    ...
    if (ant.state === 'foraging') {
      grid.homePheromone[idx] = Math.min(PHEROMONE_MAX, grid.homePheromone[idx] + PHEROMONE_DEPOSIT * 0.6)
    } else {
  ...
  // Roulette-wheel selection: choose direction probabilistically proportional to score.
  // This is the classic ACO approach — prevents greedy lock-in that causes circling.
  const totalScore = walkable.reduce((sum, s) => sum + s.score, 0)
  let pick, r = rng() * totalScore, cumul = 0
  for (let i = 0; i < walkable.length; i++) {
    cumul += walkable[i].score
    if (cumul >= r) { pick = walkable[i]; break }
  }
  if (!pick) pick = walkable[walkable.length - 1]
  ...
  if (ant.state === 'foraging') {
    // Foraging ants leave home-pheromone so returning ants can find the nest
    grid.homePheromone[i] = Math.min(PHEROMONE_MAX, grid.homePheromone[i] + PHEROMONE_DEPOSIT * 0.6)
  } else {
    // Returning ants leave strong food-pheromone, stronger near the food source
    // This creates concentrated trails that guide other foraging ants efficiently
    const depositStrength = PHEROMONE_DEPOSIT * Math.max(1.0, 2.5 - ant.stateSteps * 0.008)
    grid.foodPheromone[i] = Math.min(PHEROMONE_MAX, grid.foodPheromone[i] + depositStrength)
  }
...
function getEditButtons() {
  // Position below the HTML transport buttons (Pause/Reset/New Map are at top: 6, ~32px tall)
  const bw = 70, bh = 26, gap = 6, startX = 8, startY = 46
  return [
...
function renderCanvas(ctx, canvas, groundImg, grid, ants, foods, nestX, nestY, obstacles,
  ...
  // Edit mode overlay — positioned below the transport buttons
  if (editMode) {
    ctx.fillStyle = 'rgba(255,255,245,0.75)'
    ctx.fillRect(0, 38, w, 42)
