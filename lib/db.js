...
export async function getProject(projectId) {
  ...
  if (IS_POSTGRES) {
    ...
    project = getSqliteDb().prepare('SELECT * FROM projects WHERE id = ?').get(projectId)
    if (!project) return null
    phases = getSqliteDb().prepare('SELECT * FROM project_phases WHERE project_id = ? ORDER BY ROWID').all(projectId)
  }
...
export async function getAllProjects() {
  ...
  if (IS_POSTGRES) {
    ...
    projects = projects.map(p => ({
      ...p,
      phases: getSqliteDb().prepare('SELECT * FROM project_phases WHERE project_id = ? ORDER BY ROWID').all(p.id)
    }))
  }
...
export async function updateProjectField(projectId, field, value, userId = null) {
  ...
  if (IS_POSTGRES) {
    ...
    getSqliteDb().prepare(`UPDATE projects SET ${dbField} = ?, updated_at = ? WHERE id = ?`).run(value, now, projectId)
  }
...
export async function updatePhaseField(phaseId, field, value, userId = null) {
  ...
  if (IS_POSTGRES) {
    ...
    phase = getSqliteDb().prepare('SELECT * FROM project_phases WHERE id = ?').get(phaseId)
  }
  ...
  if (IS_POSTGRES) {
    ...
    getSqliteDb().prepare(`UPDATE project_phases SET ${dbField} = ? WHERE id = ?`).run(value, phaseId)
  }
  ...
  if (IS_POSTGRES) {
    ...
    return getSqliteDb().prepare('SELECT * FROM project_phases WHERE id = ?').get(phaseId)
  }
}
...
export async function getPhasesForProject(projectId) {
  ...
  if (IS_POSTGRES) {
    ...
    return getSqliteDb().prepare('SELECT * FROM project_phases WHERE project_id = ? ORDER BY ROWID').all(projectId)
  }
}
...
export async function advanceProjectPhase(projectId, actualDate, actualCost, userId = null) {
  ...
  if (currentPhaseRow) {
    if (IS_POSTGRES) {
      ...
      getSqliteDb().prepare(`UPDATE project_phases SET is_actual = 1, actual_date = ?, actual_cost = ? WHERE id = ?`)
  ...
  if (IS_POSTGRES) {
    ...
    getSqliteDb().prepare(`UPDATE projects SET current_phase = ?, updated_at = ? WHERE id = ?`)
...
export async function getPortfolios() {
  ...
  if (IS_POSTGRES) {
    ...
    const portfolios = getSqliteDb().prepare('SELECT * FROM portfolios ORDER BY name').all()
    return portfolios.map(p => {
      const projectCount = getSqliteDb().prepare('SELECT COUNT(*) as cnt FROM portfolio_projects WHERE portfolio_id = ?').get(p.id)
...
export async function getPortfolio(id) {
  ...
  if (IS_POSTGRES) {
    ...
    portfolio = getSqliteDb().prepare('SELECT * FROM portfolios WHERE id = ?').get(id)
    if (!portfolio) return null
    ...
    projects = projects.map(project => ({
      ...project,
      phases: getSqliteDb().prepare('SELECT * FROM project_phases WHERE project_id = ? ORDER BY ROWID').all(project.id)
    }))
  }
...
export async function updatePortfolio(id, fields, userId = null) {
  ...
  for (const [key, value] of Object.entries(fields)) {
    ...
    if (IS_POSTGRES) {
      ...
      getSqliteDb().prepare(`UPDATE portfolios SET ${dbField} = ?, updated_at = ? WHERE id = ?`).run(value, now, id)
    }
...
export async function deletePortfolio(id) {
  ...
  if (IS_POSTGRES) {
    ...
    portfolio = getSqliteDb().prepare('SELECT * FROM portfolios WHERE id = ?').get(id)
    if (!portfolio) return
    getSqliteDb().prepare('DELETE FROM portfolio_projects WHERE portfolio_id = ?').run(id)
    getSqliteDb().prepare('DELETE FROM portfolio_snapshots WHERE portfolio_id = ?').run(id)
    getSqliteDb().prepare('DELETE FROM portfolios WHERE id = ?').run(id)
  }
...
export async function addProjectToPortfolio(portfolioId, projectId, userId = null) {
  ...
  try {
    if (IS_POSTGRES) {
      ...
      getSqliteDb().prepare('INSERT INTO portfolio_projects (portfolio_id, project_id, added_at) VALUES (?, ?, ?)')
  ...
  if (IS_POSTGRES) {
    ...
    project = getSqliteDb().prepare('SELECT name, current_phase FROM projects WHERE id = ?').get(projectId)
  }
...
export async function removeProjectFromPortfolio(portfolioId, projectId, userId = null) {
  ...
  if (IS_POSTGRES) {
    ...
    project = getSqliteDb().prepare('SELECT name, current_phase FROM projects WHERE id = ?').get(projectId)
    getSqliteDb().prepare('DELETE FROM portfolio_projects WHERE portfolio_id = ? AND project_id = ?').run(portfolioId, projectId)
  }
...
export async function getProjectSnapshot(snapshotId) {
  ...
  if (IS_POSTGRES) {
    ...
    row = getSqliteDb().prepare('SELECT * FROM project_snapshots WHERE id = ?').get(snapshotId)
  }
...
export async function getPortfolioSnapshot(snapshotId) {
  ...
  if (IS_POSTGRES) {
    ...
    row = getSqliteDb().prepare('SELECT * FROM portfolio_snapshots WHERE id = ?').get(snapshotId)
  }
...
export async function getUserByEmail(email) {
  ...
  if (IS_POSTGRES) {
    ...
    return getSqliteDb().prepare('SELECT * FROM users WHERE email = ?').get(email) || null
  }
}
...
export async function getUserById(id) {
  ...
  if (IS_POSTGRES) {
    ...
    return getSqliteDb().prepare('SELECT id, email, name, created_at FROM users WHERE id = ?').get(id) || null
  }
}
